...
class Social {
    ...
    public function profile_show($l){
        ...
        $cacheFile = 'html/profile/'.$this->profileID.'_'.$l.'.html';

        if(file_exists($cacheFile)){
            require $cacheFile;
        } else {
            self::profile_showFallback($this->profileID);
        }
...
<?php
    ...
    public function profile_showFallback($id){

        $this->session->newQuery();
        $sql = "SELECT id, login, premium, lastLogin FROM users WHERE id = '".intval($id)."' LIMIT 1";
        $user = $this->pdo->query($sql)->fetch(PDO::FETCH_OBJ);

        $this->session->newQuery();
        $sql = "SELECT clanID FROM clan_users WHERE userID = '".intval($id)."' LIMIT 1";
        $clanRow = $this->pdo->query($sql)->fetch(PDO::FETCH_OBJ);
        ...
        <div class="span12">
            <div class="widget-box">
                <div class="widget-title">
                    ...
                    <h5><?php echo htmlspecialchars($user->login ?? 'Unknown'); ?></h5>
                </div>
                <div class="widget-content padding">
                    <p><b>Member since login:</b> <?php echo htmlspecialchars($user->lastlogin ?? '-'); ?></p>
                    <p><b>Clan:</b> <?php echo $clanRow ? htmlspecialchars($clanRow->clanid) : 'None'; ?></p>
                    <p><b>Premium:</b> <?php echo (!empty($user->premium)) ? 'Yes' : 'No'; ?></p>
                </div>
            </div>
        </div>
<?php
