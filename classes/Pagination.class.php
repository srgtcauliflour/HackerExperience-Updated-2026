...
<?php                    
                ...
                $cacheFile = 'html/ranking/user_'.$pageToLoad.'.html';
                ...
                if (file_exists($cacheFile)) {
                    ...
                    $sqlUsers = "SELECT ranking_user.userID, users.login, users_stats.exp,
                                        clan_users.clanID, clan.nick AS clanNick, clan.name AS clanName,
                                        (SELECT COUNT(*) FROM lists WHERE lists.userID = ranking_user.userID) AS hackedDB
                                 FROM ranking_user
                                 INNER JOIN users ON ranking_user.userID = users.id
                                 LEFT JOIN users_stats ON ranking_user.userID = users_stats.uid
                                 LEFT JOIN clan_users ON ranking_user.userID = clan_users.userID
                                 LEFT JOIN clan ON clan.clanID = clan_users.clanID
                                 WHERE ranking_user.rank >= 0
                                 ORDER BY ranking_user.rank ASC
                                 LIMIT $limit OFFSET $offset";
                    $rankStmt = $this->pdo->query($sqlUsers);
                    $i = $offset;
                    ...
                    while ($row = $rankStmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        $clanStr = '';
                        if ($row['clanid']) {
                            $clanStr = '<a href="clan?id='.(int) $row['clanid'].'">['.htmlspecialchars($row['clannick']).'] '.htmlspecialchars($row['clanname']).'</a>';
                        }
?>
                                        <tr>
                                            <td><center><?php echo $i; ?></center></td>
                                            <td><a href="profile?id=<?php echo (int) $row['userid']; ?>"><?php echo htmlspecialchars($row['login']); ?></a></td>
                                            <td><center><?php echo (int) $row['exp']; ?></center></td>
                                            <td><center><?php echo (int) $row['hackeddb']; ?></center></td>
                                            <td><?php echo $clanStr; ?></td>
                                        </tr>
<?php
                    }
...
<?php                    
                ...
                $cacheFile = 'html/ranking/clan_'.$pageToLoad.'.html';
                ...
                if (file_exists($cacheFile)) {
                    ...
                    $offset = $pageToLoad * $limit;
                    $sqlClans = "SELECT ranking_clan.clanID, clan.name, clan.nick, clan_stats.members, clan_stats.totalMemberPower AS power, clan_stats.won, clan_stats.lost
                                 FROM ranking_clan
                                 INNER JOIN clan ON ranking_clan.clanID = clan.clanID
                                 LEFT JOIN clan_stats ON clan_stats.cid = clan.clanID
                                 WHERE ranking_clan.rank > 0
                                 ORDER BY ranking_clan.rank ASC
                                 LIMIT $limit OFFSET $offset";
                    $rankStmt = $this->pdo->query($sqlClans);
                    $i = $offset;
...
?>
                                        <tr>
                                            <td><center><?php echo $i; ?></center></td>
                                            <td><a href="clan?id=<?php echo (int) $row['clanid']; ?>">[<?php echo htmlspecialchars($row['nick']); ?>] <?php echo htmlspecialchars($row['name']); ?></a></td>
                                            <td><center><?php echo (int) $row['power']; ?></center></td>
                                            <td><center><?php echo (int) $row['won']; ?> / <?php echo (int) $row['lost']; ?></center></td>
                                            <td><center><?php echo (int) $row['members']; ?></center></td>
                                        </tr>
<?php
                    }
...
<?php                   
                ...
                if($static){
                    ...
                    $cacheFile = 'html/ranking/soft_'.$pageToLoad.'.html';
                    ...
                    if (file_exists($cacheFile)) {
                        ...
                        $offset = $pageToLoad * $limit;
                        $sqlSoft = "SELECT ranking_software.softID, software.softname, software.softType, software.softversion
                                    FROM ranking_software
                                    INNER JOIN software ON ranking_software.softID = software.id
                                    ORDER BY ranking_software.rank ASC
                                    LIMIT $limit OFFSET $offset";
                        $rankStmt = $this->pdo->query($sqlSoft);
                        $i = $offset;
                        ...
                        while ($row = $rankStmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            $extension = $software->getExtension($row['softtype']);
                            $version = $software->dotVersion($row['softversion']);
                            $type = $software->int2stringSoftwareType($row['softtype']);
?>
                                        <tr>
                                            <td><center><?php echo $i; ?></center></td>
                                            <td><?php echo htmlspecialchars($row['softname'].$extension); ?></td>
...
<?php                
                ...
                $cacheFile = 'html/ranking/ddos_'.$pageToLoad.'.html';
...
<?php
                if (!file_exists($cacheFile) && $total == 0) {
                    echo '<p>'._('No DDoS attacks recorded yet.').'</p>';
                }
